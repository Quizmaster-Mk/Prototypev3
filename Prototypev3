...
<head>
<meta charset="UTF-8">
<title>Avengers vs Villains V3 Finale</title>
<style>
...
<body>
<h1>🦸 Avengers vs Villains V3 Finale</h1>
...
let difficulty="Normal", turn="narrative", comboUsed=false;
...
function startRound(choice){
  ...
  if(choice===1){ heroes.forEach(h=>h.pv+=5); pushLog("Les héros regagnent 5 PV grâce à la protection des civils."); render(); }
  turn="player";
  currentHero=0;
  currentEnemy=0;
  comboUsed=false;
...
function playerTurn(){
  if(currentHero>=heroes.length){ currentHero=0; enemyTurn(); return; }
...
function playerAttack(type){
  ...
  if(e.pv<=0){ pushLog(e.name+" est vaincu !"); currentEnemy++; if(currentEnemy>=enemies.length){ pushLog("🎉 Mission terminée !"); endGame(); return; } }
...
function enemyTurn(){
  if(currentEnemy>=enemies.length){ currentEnemy=0; playerTurn(); return; }
  const e=enemies[currentEnemy];
  const h=heroes[currentHero];
